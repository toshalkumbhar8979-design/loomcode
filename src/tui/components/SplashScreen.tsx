    ...
    <box flexDirection="column" flexGrow={1} alignItems="center" justifyContent="center" backgroundColor={ui.bg}>
      ...
      <box flexDirection="column" alignItems="center" marginBottom={1}>
        {LOOM_LOGO.map((line, i) => (
          <text key={i} fg={ui.primary} bold={i < 6}>{line}</text>
        ))}
      </box>

      <box marginTop={2} flexDirection="row">
        <text fg={ui.primary} bold>{"Build"}</text>
      ...
      <box marginTop={0}>
        <text fg={ui.fgMuted} dim>{"v1.1.0"}</text>
      </box>
      ...
      <box flexDirection="column" alignItems="center">
        ...
        <text fg={ui.fgMuted} dim>{"ctrl+p palette  ctrl+b sidebar  esc interrupt"}</text>
      </box>
